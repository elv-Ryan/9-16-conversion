...
absl::Status FaceToRegionCalculator::Process(mediapipe::CalculatorContext* cc) {
  ...
  if (!cc->Inputs().Tag(kFacesTag).Value().IsEmpty()) {
    ...
    // If enabled, keep only a single primary face per frame to avoid saliency
    // being dominated by many background/audience faces.
    int primary_index = -1;
    if (options_.primary_face_only() && !input_faces.empty()) {
      double best = -1e30;
      const double cx = 0.5;
      const double cy = 0.5;
      for (int i = 0; i < static_cast<int>(input_faces.size()); ++i) {
        const auto& det = input_faces[i];
        if (!det.has_location_data() || !det.location_data().has_relative_bounding_box()) {
          continue;
        }
        const auto& bb = det.location_data().relative_bounding_box();
        const double area = static_cast<double>(bb.width()) * static_cast<double>(bb.height());
        const double bx = static_cast<double>(bb.xmin()) + static_cast<double>(bb.width()) * 0.5;
        const double by = static_cast<double>(bb.ymin()) + static_cast<double>(bb.height()) * 0.5;
        const double d2 = (bx - cx) * (bx - cx) + (by - cy) * (by - cy);
        // Dominant term: area. Mild center bias to avoid picking edge faces.
        const double score = area - 0.15 * d2;
        if (score > best) {
          best = score;
          primary_index = i;
        }
    ...
    for (int i = 0; i < static_cast<int>(input_faces.size()); ++i) {
      if (options_.primary_face_only() && primary_index >= 0 && i != primary_index) {
        continue;
      }
      const auto& input_face = input_faces[i];
