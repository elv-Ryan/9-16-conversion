...
## Podman tagger proof of life

On March 27, 2026, we containerized the existing MediaPipe AutoFlip baseline in Podman and verified a real end to end run.

What we added:
- a `.dockerignore` to keep `data/`, `Backup/`, and local junk out of the build context
- a fixed `Dockerfile.autoflip` that builds from `vendor/mediapipe`
- a Bazel pin to `7.4.1`
- a thin tagger wrapper image in `Dockerfile.tagger`
- a stdin driven JSONL listener in `container/run_tagger.py`

What we had to fix:
- the Bazel workspace root was `vendor/mediapipe`, not the repo root
- the build needed the vendored `.bazelrc` included in the image context
- Bazel had to be pinned to the version already specified by vendored MediaPipe
- filenames with commas can break MediaPipe `input_side_packets`, so use safe filenames or symlinks for now

Current status:
- `localhost/mediapipe-autoflip:cpu` builds and runs in Podman
- `localhost/916-tagger:dev` accepts newline separated input paths on stdin
- the wrapper writes protocol style JSONL to `--output-path`
- input and output media are mounted at runtime, not baked into the image
