...
class RuntimeConfig:
    ...
    The default production mode treats every stdin media path as one already
    segmented shot. ``shot_manifest_path`` is a backup path for processing
    externally supplied shot intervals from a full video; this code never runs
    shot detection.
    """
...
def config_from_params(params: Mapping[str, Any]) -> RuntimeConfig:
    ...
    int_fields = {"imgsz", "batch_size", "coordinate_decimals"}
    number_fields = {
        "inference_fps",
