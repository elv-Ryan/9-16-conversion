...
class TransNetV2:
    ...
    def predict_frames(self, frames: torch.Tensor):
        ...
        def input_iterator():
            ...
            padded_inputs = torch.cat([start_frame] * no_padded_frames_start + [frames] + [end_frame] * no_padded_frames_end, 0)
        ...
        for inp in input_iterator():
            single_frame_pred, all_frames_pred = self.predict_raw(inp.to(self.device))
            predictions.append((single_frame_pred[0, 25:75, 0], all_frames_pred[0, 25:75, 0]))

        single_frame_pred = torch.cat([single_ for single_, all_ in predictions])
        all_frames_pred = torch.cat([all_ for single_, all_ in predictions])
...
class TransNetV2Model(nn.Module):
    ...
    def __init__(self,
        ...
        super(TransNetV2Model, self).__init__()
